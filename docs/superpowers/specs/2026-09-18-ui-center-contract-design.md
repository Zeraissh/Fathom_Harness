...
**决定**：`side` ↔ `overlay` 的边界**由预算算出**（`左栏实测宽 + CENTER_MIN_PX + RAIL_MIN_PX`），不写死数字；`SPLIT_MIN_PX = 1440` 写死（并排需要两条列各自够宽，这是产品判断不是预算）。

**勘误（2026-09-18 活页实测）**：上面算出的 **936 是算例，不是边界**。它假设左栏恰好 280px（`.sidebar { width: 280px }`），而活页上左栏实测 **292px**（1100 宽处）／312px（1439 宽处），所以**真实边界是 948**。

契约的正确表述是「边界 = **左栏实测宽** + 416 + 240」：`paintRightRail` 量左栏而不是写死它，因此边界随左栏折叠/变宽而移动。把 936 当常量用会误导——取证脚本已改成先量后算，见 `eval/persona-ux/_audit-20260918/p1-evidence.md` §3。
...
| 中 | 预算边界 – 1439 | `side` | `tabbed` | 单列互斥，tab 切换，可拖可折叠 |
| 窄 | < 预算边界 | `overlay` | `tabbed` | 不占位，退覆盖抽屉，默认收起 |

「预算边界」= 左栏实测宽 + 416 + 240（左栏 280 时是 936；活页实测左栏 292 时是 **948**，见 §4.1.3 勘误）。
...
```
700    窄档内部
预算边界 − 1   窄档最后 1px        ★预算边界（先量左栏再算；左栏 280 时是 935/936）
预算边界       中档第 1px
1100   中档内部
1439   中档最后 1px
1440   宽档第 1px        ★产品边界
...
判据：**预算边界两侧必须是两种形态；1439 与 1440 必须是两种形态。** 任一不成立即边界写错。

梯子必须**先量左栏再算**，不能抄一个固定数：左栏会随宽度变（1100 处 292px、1439 处 312px），抄死数就会探偏。实测见 `eval/persona-ux/_audit-20260918/p1-boundaries.mjs`。
