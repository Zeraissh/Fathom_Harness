...
> **✅ 已完成（2026-09-23，提交哈希见第八节 P1 提交表）**
>
> - **★ 真因与计划原文不同（这项的偏差最大）**：计划与审视报告都说「面板只有 4 条命令」。读码后不成立——`currentItems()` 一直在 merge `listExternalCommands()`，宿主早就注册了 **12 条**外部命令（设置 / 模型设置 / 消耗 / 指挥中心 / 产物 / 定时任务 / 记忆 / 全局搜索 / 本次变更 / 预览产物 / 打开网页 / 新手引导）。**真问题是排序 + 限高滚动**：`staticCommands` 把**5 条主题**排在全部外部命令之前，而面板列表限高可滚动——截图 `12-command-palette.png` 里「主题：高对比」正卡在下边缘被截断，可见列表还有下文。报告作者看到的是首屏 3 条动作 + 5 条主题，据此判"只有 4 条"。**主题是低频操作，占掉 5 个首屏槽位是排序错误。**
> - **计划点名的六条，复核结果是四条早在、两条真缺**：
>   | 命令 | 状态 |
>   |---|---|
>   | 打开设置 | 早在（`open-settings`） |
>   | 停止当前运行 | 早在（`stop-run`，运行中才出现） |
>   | 跳运行指挥中心 | 早在（`open-board`「指挥中心」） |
>   | 查看记忆面板 | 早在（`view-memory`「查看记忆」） |
>   | **切换工作目录** | **缺，本轮补** |
>   | **切换 Work/Code 脸** | **缺，本轮补** |
> - **落点**：
>   - `ui/public/features/command-palette.js`：新增导出 `PALETTE_PRIORITY`（action 0 / entry 1 / chrome 2）；`CommandSpec` 加 `keywords`、`priority`；`matchCommands` **空查询按档位稳定排序**（档位内保持声明次序）、**有查询时仍按相关性**（档位不许压过相关性，专有一条锁）；关键词参与匹配但**按分打 1 折**，别名不许盖过正牌标题；`registerPaletteCommand` 透传 `keywords`，`priority` 缺省 entry。给全部静态命令补了英文 / 拼音 / 同义词别名。
>   - `ui/public/index.html`：注册 `switch-workdir`（展开目录菜单；没有自定义菜单时退回原生 select 焦点，再退回目录选择器浮层——不留静默无反应的路径）与 `toggle-workspace-face`（两脸来回切），两条都进 action 档。
> - **测试**：`test/ui-command-palette.test.ts` 由 44 → **53 条**，新增 9 条：★ 排序（主题不许排在功能入口之前 + 高频动作仍在最前）/ ★ 排序只在空查询下生效（搜「暖炭」时 chrome 档的主题必须第一）/ ★ 六条能力全在（静态清单 + **从 `index.html` 静态分析抓外部注册 id**，因为那些注册在单测里不会真跑）/ ★ 条数 ≥10（**刻意不含主题**——主题凑数不算高频命令）/ ★ 每条命令拿自己的标题去搜必须搜得到自己 / ★ 别名参与匹配（stop、tingzhi、中止）/ ★ 别名打折可判 / 新命令带了 keywords+priority+run 的静态锁 / `registerPaletteCommand` 透传与缺省档位。另在 DOM 层加 ★ **键盘全程可达行为锁**：注册一条外部命令让列表 ≥10，只用 ArrowDown 一路走到主题**之后**那条外部命令（断言 `aria-selected`），Enter 真触发它的 `run` 并关面板——面板限高滚动，鼠标看不见的正是这些。
> - **变异验证（4 次，其中一次打脸）**：① 空查询不按档位排序 → 红 1；② 关键词不参与匹配 → 红 2；③ **别名加成改成盖过标题（`k - 1` → `k + 20`）→ 第一次全绿**——原判据用的是"搜标题时标题那条排第一"，而静态清单里没有任何命令的别名会撞上另一条的标题，判据对这个形态完全不敏感；**显式造出冲突**（甲的标题 = 乙的别名）后 → 红 1；④ 把 `switch-workdir` 的注册 id 改掉 → 红 2。
> - **判据守不住什么**：① "≥10 条"用的是**静态分析 index.html 的注册调用**，不是真机点开面板数出来的——宿主注册在 `import(...).then()` 里，注册失败（动态 import 挂了）时静态锁照样绿。② "首屏看得见几条"取决于面板的限高与实际字号，jsdom 量不出来；**本项只保证排序对，不保证首屏能装下几条**。真实视口取证仍是 P0 遗留的截图缺口。
