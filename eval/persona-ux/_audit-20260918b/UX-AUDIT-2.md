...
**同族的一条已存在的过宽口径（本刀已修，见 §6.2）：** `verifierCanExecute` 只看 bash 白名单，
不含 MCP 工具——`stm32-debug` 包未声明 `readOnlyCommands`，其核查者拿的是 MCP 探针工具，
徽标却会标「静态推导」。

### 6.2 「静态推导」误标的修复（判据从"宿主猜的名单"改成"包声明 + 实际工具面"）

**病（两例，同一类）：** 判据是宿主硬编码的一张通用可运行器名单。它猜不到**领域自带的
可运行器**（`kicad-cli` 重跑 ERC/DRC、`arm-none-eabi-*` 查符号全不在名单里），更看不见
**非 bash 的动手面**（真机核查全程靠 MCP 探针取证，bash 白名单却是空的）。于是：

| 包 | 实际手段 | 旧判据 | 实际渲染 |
|---|---|---|---|
| `stm32-debug` | MCP 探针（bash 全 deny） | 名单里没有 → 标 | ✗ 一份真机核查被判「产物未经运行验证」 |
| `kicad` | `kicad-cli` 重跑 ERC/DRC | 名单里没有 → 标 | ✗ 同上 |

**修法（`src/verifier.ts`，判据三条任一命中即"有手段"）：**
1. 白名单里放行了通用可运行器（原名单保留，含 `node18` 这类版本后缀形态）；
2. 包声明 `verify.mode="programmatic"`（"我的核查要重跑东西"——领域知识由包说）
   **且**白名单非空（挡住"声明了程序化验收却一条命令都没放行"的自相矛盾的包）；
3. 工具面上**实际**挂上了 MCP 工具（探针那类）。

判据③取"实际挂上"而非"包声明了"：对同一份 `mcp.json`，`filterMcpConfigForPack` 会按
run 级包切一刀、`selectPackTools` 再按子任务包的 `includeTools` 切一刀，没配 MCP 或没被
包放行时探针根本不在场——那时标静态是对的。口径收在一处：`verifierMeansFor(pack, tools)`
（`src/presets.ts`），四个宿主调用点共用。

**活页（脚本化宿主 + 真挂一个 MCP server）：** `test/fixtures/mcp-probe-server.mjs` 报出与
`stm32-debug` 同名的探针工具，`serve-planverif.ts` 里 `AGENT_UI_MCP=1` + `mcpConfigFile`
真连上它。四张卡：s1(consult) 两张都带「（静态推导）」、s2(python-coding) 与
s3(stm32-debug，探针在手) 都不带——**同一个 run 里三种口径同屏可见**。
反向对照：同一条测试把 `AGENT_UI_MCP` 去掉再跑一遍，s1 仍标、stm32-debug 那步也标
（探针真不在场）——证明判据读的是实际工具面，不是包声明。

**顺带**：徽标 hover 的理由文案从「白名单不含可运行器」改成「只有只读文本手段（白名单
无可运行器、也没有探针类工具）」——理由要跟判据一样宽。归属行在窄档的换行策略也一并定死：
结论词不折行、归属行唯一可缩但留 5em 下限、整行允许换行（压成单个「s」比不显示还糟）。
